...
final class WorkCoreBootstrapCompanyCommand extends Command
{
    protected $signature = 'workcore:bootstrap-company
        {--user= : Existing WorkCore user ID that will own the company}
        {--name= : Company name}
    ...
    protected $description = 'Create the first WorkCore company and owner membership for an existing WorkCore user.';
    ...
    public function handle(CreateCompany $createCompany): int
    {
        ...
        if (! $userModel::query()->whereKey((int) $userId)->exists()) {
            $this->error("WorkCore user [{$userId}] does not exist.");
        ...
        $this->line("Use request header X-Titan-Company: {$company->getKey()} for Sanctum-authenticated API calls.");
