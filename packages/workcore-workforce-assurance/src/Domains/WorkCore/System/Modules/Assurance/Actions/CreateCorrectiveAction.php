...
final class CreateCorrectiveAction implements BusinessActionHandlerContract
{
    public function __construct(private AssuranceRepositoryContract $repository) {}

    public function handle(ActionRequest $request): ActionHandlerResult
    {
        $record = $this->repository->createCorrectiveAction((string) ($request->payload['finding_public_id'] ?? ''), $request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('corrective_action', (string) ($record['public_id'] ?? $record['public_id'] ?? '')),
            events: [new PendingDomainEvent('workcore.corrective_action.created', 1, ['record' => $record])],
        );
    }
}
