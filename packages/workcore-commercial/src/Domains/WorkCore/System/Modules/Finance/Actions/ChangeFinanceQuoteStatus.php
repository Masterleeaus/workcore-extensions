...
final class ChangeFinanceQuoteStatus implements BusinessActionHandlerContract
{
    ...
    {
        $id = $this->required($request->payload, 'quote_id', 'quote_public_id');
        $state = $this->required($request->payload, 'state', 'status');
        $record = $this->repository->transitionQuote($id, $state, $request->payload, $request->companyId, $request->actorId);
