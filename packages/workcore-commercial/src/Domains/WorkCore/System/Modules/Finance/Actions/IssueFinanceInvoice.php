...
final class IssueFinanceInvoice implements BusinessActionHandlerContract
{
    ...
    public function handle(ActionRequest $request): ActionHandlerResult
    {
        ...
        $record = $this->repository->issueInvoice($id, $request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('finance_invoice', (string) ($record['id'] ?? $record['public_id'] ?? $record['invoice']['id'] ?? $record['quote']['id'] ?? '')),
            events: [new PendingDomainEvent('workcore.finance.invoice.issued', 1, ['record' => $record])],
        );
    }
