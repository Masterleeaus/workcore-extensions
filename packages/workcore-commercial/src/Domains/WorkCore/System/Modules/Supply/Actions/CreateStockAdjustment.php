...
final class CreateStockAdjustment implements BusinessActionHandlerContract
{
    public function __construct(private SupplyRepositoryContract $repository) {}

    public function handle(ActionRequest $request): ActionHandlerResult
    {
        $record = $this->repository->createAdjustment($request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('stock_adjustment', (string) $record['public_id']),
            events: [new PendingDomainEvent('workcore.supply.adjustment.created', 1, ['record' => $record])],
        );
    }
}
