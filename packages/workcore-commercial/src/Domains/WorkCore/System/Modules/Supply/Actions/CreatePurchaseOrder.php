...
final class CreatePurchaseOrder implements BusinessActionHandlerContract
{
    public function __construct(private SupplyRepositoryContract $repository) {}

    public function handle(ActionRequest $request): ActionHandlerResult
    {
        $record = $this->repository->createPurchaseOrder($request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('purchase_order', (string) $record['public_id']),
            events: [new PendingDomainEvent('workcore.supply.purchase_order.created', 1, ['record' => $record])],
        );
    }
}
