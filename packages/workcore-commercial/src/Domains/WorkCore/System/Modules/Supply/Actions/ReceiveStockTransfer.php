...
final class ReceiveStockTransfer implements BusinessActionHandlerContract
{
    ...
    public function handle(ActionRequest $request): ActionHandlerResult
    {
        ...
        $record = $this->repository->receiveTransfer((string) ($request->payload['transfer_public_id'] ?? ''), $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('stock_transfer', (string) $record['public_id']),
            events: [new PendingDomainEvent('workcore.supply.transfer.received', 1, ['record' => $record])],
        );
    }
}
