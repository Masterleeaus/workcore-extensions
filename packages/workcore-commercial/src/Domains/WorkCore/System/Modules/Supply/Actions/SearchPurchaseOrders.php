...
final class SearchPurchaseOrders
{
    public function __construct(private SupplyRepositoryContract $repository,private PermissionResolverContract $permissions){}
    public function execute(array $filters=[],int $perPage=25):LengthAwarePaginator{$tenant=workcore_tenant();$level=$this->permissions->accessLevel((int)$tenant->userId(),$tenant->companyId(),(string)config('workcore.supply.permissions.view'));abort_if($level===WorkCoreAccessLevel::None,403);return $this->repository->searchPurchaseOrders($tenant->companyId(),$filters,$perPage);}
}
