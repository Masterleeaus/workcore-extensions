...
{
    public function __construct(private PrivilegedTenantAccessContract $privileged) {}

    public function execute(string $token, int $userId, string $userEmail): CompanyMember
    {
        return $this->privileged->run(
            $userId,
            'Resolve and accept a WorkCore company invitation before tenant context exists.',
            function () use ($token, $userId, $userEmail): CompanyMember {
