...
final class RequestDocumentApproval implements BusinessActionHandlerContract
{
    public function __construct(private DocumentRepositoryContract $repository) {}

    public function handle(ActionRequest $request): ActionHandlerResult
    {
        $record = $this->repository->requestApproval((string) ($request->payload['document_public_id'] ?? ''), $request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('document_approval', (string) ($record['public_id'] ?? '')),
            events: [new PendingDomainEvent('workcore.document.approval_requested', 1, ['record' => $record])],
        );
    }
}
