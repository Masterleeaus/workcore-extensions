...
final class CheckoutAsset implements BusinessActionHandlerContract
{
    ...
    public function handle(ActionRequest $request): ActionHandlerResult
    {
        ...
        $record = $this->repository->checkoutAsset($asset, $request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('asset', $asset),
            events: [new PendingDomainEvent('workcore.asset.checked_out', 1, ['asset_public_id' => $asset, 'custody' => $record])],
        );
    }
}
