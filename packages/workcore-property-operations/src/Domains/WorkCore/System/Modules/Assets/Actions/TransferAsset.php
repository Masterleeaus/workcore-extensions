...
final class TransferAsset implements BusinessActionHandlerContract
{
    ...
    public function handle(ActionRequest $request): ActionHandlerResult
    {
        ...
        $record = $this->repository->transferAsset($asset, $request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('asset', $asset),
            events: [new PendingDomainEvent('workcore.asset.transferred', 1, ['asset_public_id' => $asset, 'transfer' => $record])],
        );
    }
}
