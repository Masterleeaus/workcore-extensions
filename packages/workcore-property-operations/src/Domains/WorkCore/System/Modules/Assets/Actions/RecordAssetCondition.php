...
final class RecordAssetCondition implements BusinessActionHandlerContract
{
    ...
    public function handle(ActionRequest $request): ActionHandlerResult
    {
        ...
        $record = $this->repository->recordCondition($asset, $request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('asset', $asset),
            events: [new PendingDomainEvent('workcore.asset.condition_recorded', 1, ['asset_public_id' => $asset, 'condition' => $record])],
        );
    }
}
