...
final class LinkPremisesDocument implements BusinessActionHandlerContract
{
    ...
    public function handle(ActionRequest $request): ActionHandlerResult
    {
        ...
        $record = $this->repository->linkDocument($premises, $payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('premises_document_link', (string) $record['public_id']),
            events: [new PendingDomainEvent('workcore.premises.document.linked', 1, ['record' => $record, 'premises_public_id' => $premises])],
        );
    }
}
