...
final class DeleteServiceFaq implements BusinessActionHandlerContract
{
    public function __construct(private ServiceCatalogueRepositoryContract $repository) {}

    public function handle(ActionRequest $request): ActionHandlerResult
    {
        $record = $this->repository->deleteFaq((string) $request->payload['faq_public_id'], $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            $record,
            new TypedReference('service_faq', (string) $record['public_id']),
            [new PendingDomainEvent('workcore.service_faq.deleted', 1, ['record' => $record])],
        );
    }
}
